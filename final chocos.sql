...
--- To put random in place of blank in team ---

update people set team="random" where team = "";
...
select spid,total,product,dense_rank() over(partition by spid order by total desc) as rnk from(
...
select p.size,p.Product,sum(s.boxes) as S_boxes,sum(s.Customers) as S_customers,sum(amount) as s_amount
...
select spid,total,dense_rank() over(order by total desc) as rnk
from (
...
select geo,product,total,dense_rank() over (partition by geo order by total desc)  as rnk from (
...
select pr.size,g.geo,sum(s.amount) as s_amount
from products pr join sales s on
pr.pid=s.PID
join geo g on g.GeoID=s.GeoId
...
select category,geo,sum(boxes) as s_boxes
from products pr join sales s on
pr.pid=s.pid
